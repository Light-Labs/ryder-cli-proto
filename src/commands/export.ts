  ...
  async run() {
    ...
    if (args.what === 'identity')
      ...
      if (args.what === 'owner_key') {
        commands.push(flags.private_key ? RyderSerial.COMMAND_EXPORT_OWNER_KEY_PRIVATE_KEY : RyderSerial.COMMAND_EXPORT_OWNER_KEY);
      } else { // app_key
        commands.push(flags.private_key ? RyderSerial.COMMAND_EXPORT_APP_KEY_PRIVATE_KEY : RyderSerial.COMMAND_EXPORT_APP_KEY);
      }
    }
